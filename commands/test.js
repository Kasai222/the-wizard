...
//mapping tables
  ...
  esm : "enemyservermod",
  sm : "enemyservermod",
  cardmultiplieroverride : "cardmultiplieroverride",
  cmv : "cardmultiplieroverride"
  };
  ...
  cardmultiplieroverride : handleArgument
}

  var oneTimeOnly = { //list of inputs that don't stack
	np : true,
	enemyclass : true,
	enemyattribute : true,
	supereffectivemod : true,
	level : true,
	strengthen : true,
	enemyhp : true,
	enemyservermod : true,
	cardmultiplieroverride : true,
	fou : true
  };
  ...
  if (arg[2]) {  //if it has a number value
    ...
    if(typeof field_handlers[field] == 'function' && !oneTimeOnly[field]) { 
      dataFromUser[field] += field_handlers[field](dataFromUser, arg[2]); //set input 
  } 
    else if(typeof field_handlers[field] == 'function') { 
      dataFromUser[field] = field_handlers[field](dataFromUser, arg[2]); //set input 
  } 
...
//consolidate some variables
 ...
 if(chain) { //TEMPORARY - 'chain' currently only set by 'bbb'
  ...
  //get passive cardmod
    ...
    if(dataFromUser["cardmultiplieroverride"] != 0) cardDamageValue = dataFromUser["cardmultiplieroverride"];
  ...
  //attribute advantage
  if (dataFromUser["enemyattribute"] != 'none') {
...
if(enemyHP > -1){
  ...
  if(inputedCardPosition == 'first' && inputedCardType == 'arts') artsFirst = 1;
  ...
  if(enemyServerMod == 1 && enemyServerModDefaults[classes[dataFromUser["enemyclass"]]]) enemyServerMod = enemyServerModDefaults[classes[dataFromUser["enemyclass"]]];
  ...
  for(i=0; i<hits; i++) {
   ...
   if(damageForMinHit >= minRollHP) overkillMin = true;
   if(damageForMaxHit >= maxRollHP) overkillMax = true;
